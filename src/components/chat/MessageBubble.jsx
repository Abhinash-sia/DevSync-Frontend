...
export default function MessageBubble({ message, isMe }) {
  ...
  return (
    <div className={`flex ${isMe ? "justify-end" : "justify-start"} ${isSending ? "animate-pulse" : ""}`}>
      <div
        className={`relative max-w-[85%] rounded-[24px] px-5 py-3.5 shadow-sm transition-opacity duration-200 ${
          isSending ? "opacity-60" : "opacity-100"
        } ${
          isMe
            ? "bg-gradient-to-br from-[var(--primary-2)]/20 to-[var(--primary-2)]/10 border border-[var(--primary-2)]/30 text-white rounded-br-sm"
      ...
      >
        ...
        >
          ...
          {isMe && (
            isSending ? (
              <span className="text-[#8ee7df]/40">●</span>
            ) : (
